...
public:
    int largestRectangleArea(vector<int>& heights) {
        ...
        return largestRectangleArea(heights.begin(), heights.end() - 1);
    }
    ...
    int largestRectangleArea(vector<int>::iterator head, vector<int>::iterator tail) {
        ...
        int rect = *minHeightIter * ( distance(head, tail) + 1 );
        if( head != minHeightIter ) rect = max( rect, largestRectangleArea(head, minHeightIter - 1));
        if( tail != minHeightIter ) rect = max( rect, largestRectangleArea(minHeightIter + 1, tail));
        return rect;
    }
};
